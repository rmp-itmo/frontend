...
class AchievementsRepoImpl: AchievementsRepository {
    override suspend fun fetchAchievements(): AchievementsDto? =
        ApiClient.authorizedRequest<AchievementsDto>(ApiClient.Method.GET, "users/stat").successOr(null)

    override suspend fun shareAchievement(shareAchievementDto: ShareAchievementDto): PostDto? =
        ApiClient.authorizedRequest<PostDto>(ApiClient.Method.POST, "social/share/achievement", shareAchievementDto).successOr(null)
}